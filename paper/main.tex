...
Paraphrase      & 0.063 [0.052, 0.074] & 0.197 [0.186, 0.209] \\
...
Our results converge on a consistent picture: in this Chinese music setting, LAION CLAP's audio-text alignment is substantially influenced by cross-modal correlation structure and acoustic properties, with semantic content contributing far less. An important question is whether this insensitivity reflects a \emph{choice} (CLAP has semantic capacity but ignores it) or a \emph{limitation} (CLAP's text encoder never acquired robust semantic representations). CLAP's text encoder is a much smaller transformer than LLMs, trained via contrastive objectives on audio-text pairs rather than through language modeling. The probing classifier result (CLAP achieves only 71.2\% accuracy distinguishing original from hachimi text, compared to BERT's 90.3\%) supports the latter interpretation: CLAP's text encoder encodes surface-level features insufficient for semantic discrimination. If the issue is representational capacity rather than alignment objective, then scaling up text encoders or using stronger pretraining may change the balance between semantic and acoustic contributions.

\paragraph{Why does this matter?}
These findings have practical implications for music information retrieval systems that use CLAP for zero-shot classification or text-to-music retrieval. If alignment is dominated by acoustic properties and phonological regularity rather than semantics, queries that differ in phonological structure from their target audio will systematically underperform. The paraphrase result is particularly relevant: LLM-generated text descriptions may align poorly with audio even when semantically correct, because they lack the phonological and rhythmic properties of natural song text. Nonsense syllables with high rhythmic regularity align better than meaningful lyrics, suggesting that text-audio retrieval may capture text-music \emph{genre} associations rather than semantic understanding.
